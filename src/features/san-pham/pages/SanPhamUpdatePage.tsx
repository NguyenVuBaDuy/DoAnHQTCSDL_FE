...
const SanPhamUpdatePage = () => {
  ...
  useEffect(() => {
    ...
    if (cat) {
      if (cat.maDmCha) {
        setSelectedParentId(cat.maDmCha);
        // Delay setting child so the child options are available
        setTimeout(() => setSelectedChildId(cat.maDm), 100);
      } else {
        setSelectedParentId(cat.maDm);
      }
    }
    ...
    if (product.variants && product.variants.length > 0) {
      setVariants(
        product.variants.map((v) => ({
          ...
          sku: v.sku || "",
          barcode: v.barcode || "",
          trangThai: v.trangThai || "DangBan",
        })),
      );
    }

    setIsInitialized(true);
  }, [detailResponse, isInitialized, isCatLoading]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast
      .promise(sanPhamService.updateSanPham(Number(maSp), payload), {
  ...
  if (isDetailLoading || isCatLoading) {
    return (
...
export default SanPhamUpdatePage;
